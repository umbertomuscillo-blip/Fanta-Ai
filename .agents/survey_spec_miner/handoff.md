# HANDOFF REPORT: FANTACALCIO SPECIFICATION MINING
...
2. **R2 (Lineup Optimizer)**:
   ...
   - Goalkeeper difficulty rating from `goalkeeper_analyzer.py` calculates expected conceded goals $xGC = 0.55 \cdot 	ext{Diff}_{GK} + 0.15 \cdot rac{10 - 	ext{DEF\_RATING}}{4.0}$.
   - Defense Modifier rule: Active $\iff n_D \ge 4$. Calculates $M_{dif} = rac{V_{GK} + V_{D,(1)} + V_{D,(2)} + V_{D,(3)}}{4}$.
...
3. **R3 (Moneyball Index)**:
   ...
   - $MVI(p) = rac{VORP(p) \cdot 100}{\max(1, 	ext{PrezzoTarget}(p))}$.
...
| 10 | R2: Lineup Optimizer | Goalkeeper playing Away vs Inter (Tier 1) | Classified as Difficile (Tier 3), $xGC pprox 1.8$, clean sheet prob $pprox 6\%$. |
